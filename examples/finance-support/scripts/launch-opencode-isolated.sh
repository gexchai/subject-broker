...
if test "$(uname -s)" != "Darwin" || ! test -x /usr/bin/sandbox-exec; then
  echo "The isolated OpenCode reference launcher requires macOS sandbox-exec." >&2
  exit 1
fi
...
node "$repo_root/dist/isolated-opencode.js" \
  --subject "$subject_id" \
  --policy "$policy_path" \
  --audit "$audit_path" \
  --workspace "$workspace" \
