...
| Can the implemented broker boundary be checked without relying on model behavior? | Yes. The finance/support conformance command directly exercises MCP calls and emits a layer-labelled JSON report. Harness and host isolation remain explicitly `not-provided`. | ADR-025 and ADR-026; `src/conformance.ts`; `tests/integration/conformance-cli.test.ts` |
| Can a reference deployment block direct same-user reads while preserving an authorized broker path? | Yes, in the exact ADR-027 macOS topology. The model-free differential blocks direct, symlink-aliased, policy, and audit reads while the external subject-bound broker remains functional. This is not portable production sandboxing or non-transferable identity. | ADR-027; `src/host-conformance.ts`; `tests/integration/host-isolation.test.ts` |
...
| --- | --- |
| What supported portable sandbox should replace the deprecated macOS reference mechanism? | A maintained container, VM, App Sandbox, or separate-identity deployment with the same direct-deny/broker-allow differential and operational requirements. |
