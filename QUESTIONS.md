...
| Which adapter follows Claude Code? | Hermes, native Pi, Codex CLI 0.144.4, and the blocked VS Code local Copilot 1.130.0 setup have been classified to the extent their shipped mechanisms allow. OpenCode is the next candidate with a real delegation/tool-filtering surface. Gemini CLI remains deferred until user demand justifies it. |
